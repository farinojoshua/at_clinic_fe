import { useEffect, useState } from "react";
// import axios from "axios";
import BgDokter from "../assets/poli-x.jpg";
import DefaultFoto from "../assets/326.png";
import dummyData from "../data/dokter.json";

export default function DokterList() {
  const [dokterList, setDokterList] = useState([]);
  const spesialisFilter = "Umum"; // Ganti ini kalau mau filter "Kandungan", "Anak", dll

  useEffect(() => {
    // Filter dari data dummy
    const filtered = dummyData.filter(
      (dokter) =>
        dokter.spesialis.toLowerCase() === spesialisFilter.toLowerCase()
    );
    setDokterList(filtered);

    // Kalau pakai API:
    // axios.get(`http://localhost:3000/api/doctors?spesialis=${spesialisFilter}`)
    //   .then(res => setDokterList(res.data))
    //   .catch(err => console.error("Gagal ambil data dokter:", err));
  }, []);

  return (
    <div className="bg-white min-h-screen">
      {/* Background */}
      <section className="w-full h-[200px] md:h-[280px] overflow-hidden">
        <img
          src={BgDokter}
          alt="Background Daftar Dokter"
          className="w-full h-full object-cover"
        />
      </section>

      {/* List Dokter & Judul di dalam Card */}
      <section className="relative z-10 px-4 pb-16">
        <div className="max-w-5xl mx-auto -mt-20 md:-mt-24 bg-white/90 backdrop-blur-md shadow-2xl rounded-3xl px-6 py-10 md:py-14">
          {/* Judul */}
          <div className="text-center mb-10">
            <h2 className="inline-block bg-white text-2xl font-bold text-blue-600 px-6 py-3 rounded-2xl">
              Poli {spesialisFilter}
            </h2>
          </div>

          <div className="space-y-12">
            {dokterList.map((dokter, i) => (
              <div
                key={i}
                className="bg-white shadow-xl rounded-3xl overflow-hidden flex flex-col md:flex-row items-stretch"
              >
                {/* Foto & Nama */}
                <div className="bg-[#007BFF] w-full md:w-[260px] p-8 flex flex-col items-center justify-center text-white">
                  <img
                    src={dokter.foto || DefaultFoto}
                    alt={dokter.nama}
                    className="w-32 h-32 md:w-36 md:h-36 rounded-full object-cover border-4 border-white shadow-lg"
                  />
                  <h3 className="text-lg font-semibold mt-4 text-center">
                    <span className="text-white font-medium">dr.</span>{" "}
                    <span className="text-white">
                      {dokter.nama.replace("dr. ", "")}
                    </span>
                  </h3>
                  <p className="text-sm text-white/90 mt-1">
                    {dokter.spesialis}
                  </p>
                </div>

                {/* Jadwal */}
                <div className="flex-1 p-6 md:p-10 flex flex-col justify-between">
                  <div>
                    <h4 className="text-blue-600 font-bold text-center md:text-left mb-4 text-base">
                      Jadwal
                    </h4>
                    <div className="overflow-x-auto">
                      <table className="w-full text-sm text-gray-800 text-center">
                        <thead>
                          <tr className="font-semibold text-gray-500 border-b">
                            {dokter.jadwal.map((j, idx) => (
                              <th
                                key={idx}
                                className="px-2 py-1 whitespace-nowrap"
                              >
                                {j.hari}
                              </th>
                            ))}
                          </tr>
                        </thead>
                        <tbody>
                          <tr className="font-medium">
                            {dokter.jadwal.map((j, idx) => (
                              <td
                                key={idx}
                                className="px-2 py-2 whitespace-nowrap"
                              >
                                {j.jam}
                              </td>
                            ))}
                          </tr>
                        </tbody>
                      </table>
                    </div>
                  </div>
                  <div className="mt-6 text-center md:text-right">
                    <button className="bg-blue-500 hover:bg-blue-600 text-white px-5 py-2 text-sm rounded-full transition">
                      Buat Janji
                    </button>
                  </div>
                </div>
              </div>
            ))}
          </div>
        </div>
      </section>
    </div>
  );
}
