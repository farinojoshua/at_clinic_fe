...
export default function Poli() {
  ...
  useEffect(() => {
    axios
      .get("https://982f-103-132-239-226.ngrok-free.app/api/specialist", {
        headers: {
          "ngrok-skip-browser-warning": "true",
        },
      })
      .then((res) => {
        console.log("Response API:", res.data); // Tambahkan ini
        setPoliList(res.data.data); // Harus pastikan `res.data.data` itu array
        setLoading(false);
      })
      .catch((err) => {
        console.error("Gagal fetch data spesialis:", err);
        setLoading(false);
      });
  }, []);
          ...
          {loading ? (
            ...
            <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-y-4 gap-x-4 px-2'>
              {poliList.map((poli) => (
                <div
                  key={poli.id}
                  onClick={() => navigate(`/dokter/${poli.id}`)}
